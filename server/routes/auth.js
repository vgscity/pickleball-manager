...
router.post('/login', async (req, res) => {
  ...
  const rows = await query('SELECT * FROM admins WHERE username = $1', ['admin']);
  const admin = rows[0];
...
router.post('/change-password', authMiddleware, async (req, res) => {
  ...
  await query('UPDATE admins SET password_hash=$1 WHERE id=$2', [bcrypt.hashSync(newPassword, 10), req.admin.id]);
  res.json({ ok: true });
});
